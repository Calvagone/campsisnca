...
test_that("Times must be monotonically increasing", {
  ...
  expect_error(AUC("Y") %>% campsisnca::calculate(campsis), regexp="Times must be monotonically increasing")
})
...
test_that("Times can't be NA's", {
  ...
  expect_error(AUC("Y") %>% campsisnca::calculate(campsis), regexp="Sample times cannot be NA")
})
...
test_that("Observations can't be NA's", {
  ...
  expect_error(AUC("Y") %>% campsisnca::calculate(campsis), regexp="Observations at times '10' are NA")
})
