...
test_that("Auto-replace of known NCA metrics works as expected", {
  ...
  expect_equal(custom1@custom_function, "rlang::as_function(~i_value(Cmax(),.x,.y) > 12)")
  
  custom2 <- CustomMetric(fun=~Cmax > 12 & Ctrough < 5, name="Cmax > 12 & Ctrough < 5", categorical=TRUE)
  custom2 <- custom2 %>%
    replaceAll(pattern=NCAMetrics(), replacement="auto")
  expect_equal(custom2@custom_function, "rlang::as_function(~i_value(Cmax(),.x,.y) > 12 & i_value(Ctrough(),.x,.y) < 5)")
  ...
  custom4 <- custom4 %>%
    replaceAll(pattern=NCAMetrics(), replacement="auto")
  expect_equal(custom4@custom_function, "rlang::as_function(~i_value(AUC(),.x,.y) > 100)")
})
