...
test_that("Dataset 1 - time above 0.75 at day 1", {
  ...
  output_regression_test(results1@individual, file = get_ref_file("dataset1_time_above_0_75.csv"))
})
...
test_that("Dataset 1 - time below 0.75 at day 1 (check reciprocity)", {
  ...
  output_regression_test(results2@individual, file = get_ref_file("dataset1_time_above_0_75.csv"))
})
...
test_that("Method 'computeTimeAboveBelow' deals well with special cases", {
  ...
  expect_error(
    computeTimeAboveBelow(x1 = 0, x2 = 0, y1 = 0, y2 = 1, above = TRUE, strictly = TRUE),
    msg = "Sample times must be unique"
  )
