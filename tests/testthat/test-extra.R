...
test_that("Summary stats on categorical data only should work as expected", {
  getCategory <- ~ dplyr::case_when(
    Cmax < 10 ~ "(1) < 10 ng/mL",
    Cmax >= 10 & Cmax <= 15 ~ "(2) 10-15 ng/mL",
    Cmax > 15 ~ "(3) > 15 ng/mL"
  )
  ...
  output_regression_test(
    data = individual[1:20, ] %>% dplyr::rename(Categories = `Cmax categories`),
    file = get_ref_file("categorical_data_individual.csv")
  )
...
test_that("Method stat_display_string works as expected on categorical data when digits is provided", {
  ...
  custom <- CustomMetric(
    ...
    fun = ~ Cmax > 10,
    stat_display = "{p}",
    digits = ~ style_percent(.x, digits = 1, suffix = '%'),
    categorical = TRUE
  )
  custom <- custom %>% campsisnca::calculate(campsis_)
  expect_equal(custom %>% campsisnca::stat_display_string(), c("FALSE: 43.7%", "TRUE: 56.3%"))
  ...
  custom <- CustomMetric(
    ...
    fun = ~ Cmax > 10,
    stat_display = "{p}",
    digits = ~ style_percent(.x, digits = 2, suffix = '%'),
    categorical = TRUE
  )
  custom <- custom %>% campsisnca::calculate(campsis_)
...
test_that("Time unit of AUC can be customised", {
  ...
  table <- NCATable(json = file.path(TEST_FOLDER, "json_examples", "nca_table_8.json"))
})
...
test_that("Method NCATableOutfun can be used in campsisnca", {
  table <- NCATable(json = file.path(TEST_FOLDER, "json_examples", "nca_table_9.json"))
  subjects <- 100
  model <- model_suite$pk$`2cpt_fo`
...
test_that("Method summarise_replicates can be used to summarise Campsisnca output across replicates", {
  ...
  table <- NCATable(
    json = file.path(TEST_FOLDER, "json_examples", "nca_table_10.json")
  )
  subjects <- 100
  model <- model_suite$pk$`2cpt_fo`
  ...
  rep_nca_table <- ReplicatedNCATable(
    json = file.path(TEST_FOLDER, "json_examples", "replicated_nca_table_10.json")
  )
