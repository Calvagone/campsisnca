...
test_that("Import time range from JSON", {
  ...
  time_range <- loadFromJSON(TimeRange(), JSONElement(json$nca_analyses[[1]]$time_range))
  expect_equal(time_range, TimeRange(0, 24))
  
  time_range <- loadFromJSON(TimeRange(), JSONElement(json$nca_analyses[[2]]$time_range))
  expect_equal(time_range, TimeRange(144, 168))
})
