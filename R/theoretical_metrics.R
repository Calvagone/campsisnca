...
metrics.2cpt <- function(x, map=character(0)) {
  ...
  x <- x %>% dplyr::mutate(A=1, B=.data$K20+.data$K23+.data$K32, C=.data$K20*.data$K32, DETER=.data$B^2-4*.data$A*.data$C)
  x <- x %>% dplyr::mutate(ALPHA=(.data$B+sqrt(.data$DETER))/2)
  x <- x %>% dplyr::mutate(BETA=(.data$B-sqrt(.data$DETER))/2)
  x <- x %>% dplyr::mutate(COEFF_A=.data$DOSE*.data$KA*(.data$K32-.data$ALPHA)/(.data$V2*(.data$BETA-.data$ALPHA)*(.data$KA-.data$ALPHA)))
  x <- x %>% dplyr::mutate(COEFF_B=.data$DOSE*.data$KA*(.data$K32-.data$BETA)/(.data$V2*(.data$ALPHA-.data$BETA)*(.data$KA-.data$BETA)))
  x <- x %>% dplyr::mutate(COEFF_C=.data$DOSE*.data$KA*(.data$K32-.data$KA)/(.data$V2*(.data$ALPHA-.data$KA)*(.data$BETA-.data$KA)))
