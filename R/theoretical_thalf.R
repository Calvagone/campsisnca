...
check_map <- function(map, thalf.1cpt = TRUE) {
  ...
  assertthat::assert_that(
    is.character(map),
    msg = paste0(
      "map must be a character vector and may contain the following keys: ",
  ...
  assertthat::assert_that(
    all(check),
    msg = paste0("Unnecessary keys detected in map vector: ", paste0(keys[!check], collapse = ", "))
  )
  return(map)
}
...
#' @rdname calculate
setMethod("calculate", signature = c("theoretical_thalf_metric"), definition = function(object, x, options, ...) {
